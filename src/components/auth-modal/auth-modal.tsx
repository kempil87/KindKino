    ...
    <FormProvider {...methods}>
      <Modal name="auth" onClose={onClose}>
        {resetPasswordVisible ? (
          <div className='text-2xl flex items-center space-x-4 transition-all font-normal'>
            <button className='group' onClick={() => setResetPasswordVisible(false)}>
              <Icon className='group-hover:-translate-x-3 transition-all' name='arrow_long' size={21} />
            </button>

            <span>Восстановление пароля</span>
          </div>
        ) : (
          <div className="flex mb-4 items-end space-x-4 h-8">
            <button
              className={cc(['text-2xl transition-all font-normal', {'text-lg text-white/70': !authType}])}
        ...
        {resetPasswordVisible && <ResetPasswordContainer returnAuthFields={returnAuthFields}/>}
      </Modal>
    </FormProvider>
  );
};
